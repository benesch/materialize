...
unsafe fn read_datum<'a>(data: &'a [u8], offset: &mut usize) -> Datum<'a> {
    ...
    match tag {
        ...
        Tag::Decimal => {
            let s = i128::from_le_bytes(read_byte_array(data, offset));
            Datum::Decimal(Significand::new(s))
        }
        Tag::BytesTiny
        | Tag::BytesShort
        | Tag::BytesLong
        ...
        Tag::APD => {
            let digits = u32::from_le_bytes(read_byte_array(data, offset));
            let exponent = i32::from_le_bytes(read_byte_array(data, offset));
    ...
    D: Vector<u8>,
{
    match datum {
        ...
        Datum::Decimal(s) => {
            data.push(Tag::Decimal.into());
            data.extend_from_slice(&s.as_i128().to_le_bytes());
        }
        Datum::Bytes(bytes) => {
