...
impl<'a> CopyTextFormatParser<'a> {
    pub fn new(params: &'a CopyTextFormatParams, data: &'a [u8]) -> Self {
        Self {
            params,
            data,
            position: 0,
    ...
    fn is_column_delimiter(&self) -> bool {
        self.check_bytes(&[self.params.delimiter])
    }

    pub fn expect_column_delimiter(&mut self) -> Result<(), io::Error> {
        if self.consume_bytes(&[self.params.delimiter]) {
            Ok(())
        } else {
            Err(io::Error::new(
                io::ErrorKind::InvalidData,
    ...
    fn consume_null_string(&mut self) -> bool {
        if self.params.null.is_empty() {
            ...
            self.consume_bytes(self.params.null.as_bytes())
        }
    }
...
#[derive(Debug)]
pub enum CopyFormatParams {
    Text(CopyTextFormatParams),
    Csv(CopyCsvFormatParams),
}

pub fn decode_copy_format(
    data: &[u8],
...
pub const DEFAULT_COPY_TEXT_FORMAT_NULL: &str = "\\N";
pub const DEFAULT_COPY_TEXT_FORMAT_DELIMITER: u8 = b'\t';

#[derive(Debug)]
pub struct CopyTextFormatParams {
    pub null: String,
    pub delimiter: u8,
}

impl Default for CopyTextFormatParams {
    fn default() -> CopyTextFormatParams {
        CopyTextFormatParams {
            null: DEFAULT_COPY_TEXT_FORMAT_NULL.into(),
            delimiter: DEFAULT_COPY_TEXT_FORMAT_DELIMITER,
        }
    }
}
...
pub fn decode_copy_format_text(
    data: &[u8],
    column_types: &[mz_pgrepr::Type],
    params: CopyTextFormatParams,
) -> Result<Vec<Row>, io::Error> {
    let mut rows = Vec::new();

    let mut parser = CopyTextFormatParser::new(&params, data);
...
pub const DEFAULT_COPY_CSV_FORMAT_NULL: &str = "";
pub const DEFAULT_COPY_CSV_FORMAT_DELIMITER: u8 = b',';
pub const DEFAULT_COPY_CSV_FORMAT_QUOTE: u8 = b'"';

#[derive(Debug)]
pub struct CopyCsvFormatParams {
    pub delimiter: u8,
    pub quote: u8,
    pub escape: u8,
    pub header: bool,
...
mod tests {
    ...
    fn test_copy_format_text_parser() {
        ...
        let text = "\t\\nt e\t\\N\t\n\\x60\\xA\\x7D\\x4a\n\\44\\044\\123".as_bytes();
        let mut parser = CopyTextFormatParser::new(&params, text);
    ...
    fn test_copy_format_text_empty_null_string() {
        ...
        let params = CopyTextFormatParams { null: "".into(), delimiter: b'\t' };
        let mut parser = CopyTextFormatParser::new(&params, text);
        for line in expect {
    ...
    fn test_copy_format_text_parser_escapes() {
        ...
        for test in tests {
            let params = CopyTextFormatParams::default();
            let mut parser = CopyTextFormatParser::new(&params, test.input.as_bytes());
            assert_eq!(
                parser
                    .consume_raw_value()
                    .expect("unexpected error")
